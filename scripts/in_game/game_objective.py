...
class KOTH:
    def __init__(self,player:"_Player",objective_length:int,location_x:int,location_z:int):
        ...
        self.obj_wall1 = Entity(model="cube", scale=(self.objective_length+0.5,2,0.5), color=color.yellow, collider = "box", x=self.location_x, z=location_z - self.objective_length/2)
        self.obj_wall2 = Entity(model="cube", scale=(self.objective_length+0.5,2,0.5), color=color.yellow, collider = "box", x=self.location_x, z=location_z + self.objective_length/2)
        self.obj_wall3 = Entity(model="cube", scale=(self.objective_length+0.5,2,0.5), color=color.yellow, collider = "box", x=self.location_x - self.objective_length/2, z=location_z, rotation_y=90)
        self.obj_wall4 = Entity(model="cube", scale=(self.objective_length+0.5,2,0.5), color=color.yellow, collider = "box", x=self.location_x + self.objective_length/2, z=location_z, rotation_y=90)
